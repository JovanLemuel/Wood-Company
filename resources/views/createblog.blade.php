...
@section('content')
    <div class="text-center pt-20 py-12 px-6 mt-12">
        <h1 class="font-display font-bold text-5xl mb-6">Create Blog</h1>
    </div>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-12 max-w-4xl mx-auto">
            <form action="{{ route('blogs.store') }}" method="POST" enctype="multipart/form-data"
                ...
                <div class="mb-3">
                    ...
                    <input type="text" name="blog_content" class="mt-2 w-full border-none text-sm p-4 bg-gray-100 text-gray-700">
                </div>
                <div class="mb-3">
                    <label for="" class="block text-base tracking-tight text-gray-600">Type</label>
                    <input type="text" name="blog_type" class="mt-2 w-full border-none text-sm p-4 bg-gray-100 text-gray-700">
                </div>
                <div class="mb-3">
                ...
                <div class="mb-3">
                    ...
                    <select name="genre_id" id="" class="mt-2 form-select w-full py-3">
                        @foreach ($genres as $gr)
                            <option value="{{ $gr['id'] }}">{{ $gr->genre_name }}</option>
                        @endforeach
                    </select>
                </div>
                {{-- <div class="mb-3">
